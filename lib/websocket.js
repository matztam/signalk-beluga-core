...
class Ws {
  ...
  start () {
    ...
    if (typeof app.signalk?.on === 'function') app.signalk.on('delta', this._deltaHandler)
  ...
  // ── Sync channel ─────────────────────────────────────────────────────────
  // Relays messages between all connected sync clients (iPhone, iPad, ...) so
  // shared state (device selection, route hash) propagates to every device.
  // Without the relay the channel is accepted but inert, and two apps running
  // against the same core drift apart.
  _handleSync (ws) {
    const { app } = this.ctx
    this._syncClients.add(ws)

    const cleanup = () => this._syncClients.delete(ws)
    ws.on('close', cleanup)
    ws.on('error', cleanup)
    ...
    ws.on('message', raw => {
      try {
        ...
        for (const client of this._syncClients) {
  ...
  stop () {
    if (this._deltaHandler) {
      if (typeof this.ctx.app.signalk?.removeListener === 'function') this.ctx.app.signalk.removeListener('delta', this._deltaHandler)
      this._deltaHandler = null
    }
    if (this._aisTimer) { clearInterval(this._aisTimer); this._aisTimer = null }
    if (this._wss)      { this._wss.close(); this._wss = null }
    this._events.removeAllListeners()
    this._syncClients.clear()
  }
}
