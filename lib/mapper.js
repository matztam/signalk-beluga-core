...
function buildSensorDelta(app, deviceId) {
  ...
  // ── Water Temperature ─────────────────────────────────────────────────────
  const waterTemp = envData.water?.temperature?.value
  if (waterTemp != null) {
    values[`environment.temperature.${NAV_SRC}.0.temperature`] = waterTemp
    values[`environment.temperature.${NAV_SRC}.0.source`] = 0
  }

  // ── Outside Temperature ───────────────────────────────────────────────────
  const outsideTemp = envData.outside?.temperature?.value
  if (outsideTemp != null) {
    values[`environment.temperature.${NAV_SRC}.1.temperature`] = outsideTemp
    values[`environment.temperature.${NAV_SRC}.1.source`] = 1
  }

  // ── Inside Temperature ────────────────────────────────────────────────────
  const insideTemp = envData.inside?.temperature?.value
  if (insideTemp != null) {
    values[`environment.temperature.${NAV_SRC}.2.temperature`] = insideTemp
    values[`environment.temperature.${NAV_SRC}.2.source`] = 2
  }
