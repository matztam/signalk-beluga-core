...
const UI_PORT     = 8080
const UPDATE_PORT = 8090

// Matches libconfig's `version = "x.y.z";` inside sw-description, e.g.
//   software = { version = "2026.29.1"; imx6ul-var-dart = { ... } };
const UPDATE_VERSION_RE = /\bversion\s*=\s*"([^"]+)"/
...
class Api {
  ...
  start () {
    ...
    rest.get('/v1/settings/*splat', (req, res) => res.json({ data: {} }))
    ...
    // Sync key-value store — multi-device coordination. The hub acts as a relay:
    // REST stores/retrieves keyed state, the WS /v1/sync channel broadcasts live.
    // All keys return null when nothing has been stored (e.g. no active route).
    // Note the response shape is {value: …}, not the {data: …} used elsewhere —
    // falling through to the catch-all below would answer {data:{}} and the app
    // would read that as a stored value rather than an empty slot.
    rest.get('/v1/sync/*splat', (req, res) => {
      const key = req.path.slice('/v1/sync/'.length)
      res.json({ value: this._syncStore.get(key) ?? null })
    })
    rest.put('/v1/sync/*splat', (req, res) => {
      const key = req.path.slice('/v1/sync/'.length)
      if (req.body?.value !== undefined) this._syncStore.set(key, req.body.value)
      res.json({ value: {} })
    })

    rest.post('/v1/remote-access/token', (req, res) => res.json({ value: {} }))
    ...
    const ui = express()
    ui.get('/info', (req, res) => {
      const version = this.ctx.firmwareVersion
      res.json({ uptime: '9999', version, version_fw: version, name: deviceName })
    })
    ui.use((req, res) => res.json({ data: {} }))
    this._listen(ui, UI_PORT)

    this._startUpdateServer()
  }

  // ── Firmware "update" :8090 ─────────────────────────────────────────────
  //
  // The real Core flashes what the app POSTs here as multipart/form-data — a
  // .swu (SWUpdate) container, itself a cpio archive whose first entry is a
  // small sw-description text file with `version = "x.y.z";` near its start.
  // We only want that string, not the multi-hundred-MB image behind it, so
  // this reads raw chunks off the socket — not express.json()/a body parser,
  // both of which would buffer the whole upload before we ever saw a byte —
  // and destroys the connection the moment the version has been found.
  //
  // The app is left mid-upload with a dropped connection and reports the
  // update as failed, which is honest: nothing was actually flashed. Fixing
  // that (reading the request to completion and answering 200) only trades
  // an accurate error for a false "success", for an update that never
  // happened either way.
  _startUpdateServer () {
    const server = http.createServer((req, res) => {
      ...
      req.on('data', (chunk) => {
        ...
        // sw-description is near the top of the first part; stop growing the
        // buffer once we've seen more than enough of it without a match.
        if (buf.length > 8192 && !UPDATE_VERSION_RE.test(buf.toString('latin1'))) {
          req.destroy()
          return
        }
        const m = buf.toString('latin1').match(UPDATE_VERSION_RE)
        if (!m) return
        req.destroy()
        this._applyFirmwareVersion(m[1])
      })
      req.on('aborted', () => {}) // expected: we're the ones destroying it
      req.on('error', () => {})
    })
    this._listen(server, UPDATE_PORT)
  }

  _applyFirmwareVersion (version) {
    const { app, options } = this.ctx
    if (version === this.ctx.firmwareVersion) return
    app.debug(`Firmware update seen: ${this.ctx.firmwareVersion} → ${version}`)
    this.ctx.firmwareVersion = version
    app.savePluginOptions({ ...options, firmwareVersion: version }, () => {})
  }
