...
//
// Doppler and history are deliberately not intensities:
//   - doppler approaching/receding map to full intensity so moving targets
//     still paint; the app has no doppler channel we can address.
...
function buildPixelLut (cap) {
  ...
  if (!Array.isArray(pixels) || !pixels.length) {
    for (let i = 0; i < 256; i++) lut[i] = Math.round((Math.min(i, max) * 255) / max)
    return lut
  }
  for (let i = 0; i < 256; i++) {
    const type = pixels[i] && pixels[i].type
    if (type === 'normal') lut[i] = Math.round((Math.min(i, max) * 255) / max)
    else if (type === 'dopplerApproaching' || type === 'dopplerReceding') lut[i] = 255
    else lut[i] = 0
  }
  return lut
}
...
class Radar {
  ...
  async _flush (radarId) {
    ...
    try {
      // zstd is a native addon. If its install script was blocked or the build
      // failed there is no binding, every frame throws here, and the app shows
      // an empty overlay with no other symptom — so this is reported, not
      // debug-logged.
      if (!this._zstd) this._zstd = require('@mongodb-js/zstd')
      ...
      const frame  = await this._zstd.compress(Buffer.from(packed.buffer, packed.byteOffset, packed.byteLength))
  ...
  async _discover () {
    ...
    try {
      ...
      radars = await Promise.all(entries.map(async ([id, info]) => {
        ...
        return { id, ...info, spokesPerRevolution, maxSpokeLen, ranges, modes, pixelLut }
      }))
    } catch (e) {
  ...
  _orcaStatus (r) {
    ...
    return {
      ...
      rain:         num('rain'),
      rain_auto:    auto('rain'),
      doppler_mode: num('doppler'),
      doppler:      num('doppler') > 0,
    }
  }
  ...
  _forwardCommand (radarId, cmd) {
    ...
    if (cmd.doppler_mode != null) put('doppler', { value: cmd.doppler_mode })
    else if (cmd.doppler != null) put('doppler', { value: cmd.doppler ? 1 : 0 })
  }
}
