...
// node:zlib's zstdCompress produces frames the ORCA app's decoder cannot
// read — confirmed live: switching to it made the app crash every time the
// radar started transmitting, and reverting to @mongodb-js/zstd fixed it
// immediately. The frames differ in the header (@mongodb-js/zstd sets
// Content_Size in the descriptor, node:zlib's convenience wrapper does not),
// which is the likely cause but wasn't tracked further since the app itself
// is opaque. So: no built-in fallback, @mongodb-js/zstd only. Still a lazy
// require — the module load would crash the whole plugin if the binding
// was never built.
let _compress = null
