...
// zstd-wasm, not @mongodb-js/zstd or node:zlib:
//   - @mongodb-js/zstd is a native addon whose postinstall fetches/builds the
//     binding; npm 12+ blocks dependency install scripts by default, so the
//     binding is silently never built and every frame throws (issue #10).
//   - node:zlib's zstdCompress avoids that, but produces a different frame
//     header (missing Content_Size in the descriptor) that the ORCA app's
//     decoder cannot read — confirmed live, it crashed the app on every
//     transmit.
// @bokuweb/zstd-wasm wraps the same libzstd compiled to WASM: no native
// addon, nothing for --ignore-scripts to block, and byte-identical frames to
// @mongodb-js/zstd (verified). init() is async and must resolve before the
// first compress() call; cached so later flushes don't pay for it again.
const { init: initZstd, compress: zstdCompress } = require('@bokuweb/zstd-wasm')
let _zstdReady = null
function getCompress () {
  if (!_zstdReady) _zstdReady = initZstd()
  return async (buf) => {
    await _zstdReady
    return Buffer.from(zstdCompress(buf, 3))
  }
}
...
class Radar {
  ...
  _startWs () {
    ...
    wss.on('connection', (ws, req) => {
      ...
      if (spokeM) {
        ...
        this._openSource(radarId)
        const cleanup = () => {
          const set = this._clients.get(radarId)
          if (set) { set.delete(ws); if (set.size === 0) this._closeSource(radarId) }
        }
        ws.on('close', cleanup)
        ws.on('error', cleanup)
      } else if (statusM) {
  ...
  // Discovery registers the radar and starts its state poll; the spoke stream
  // itself is opened lazily by _openSource, only while an app client is
  // attached. mayara counts every spoke subscriber as someone watching the
  // radar and keeps it transmitting for them, so a stream held open around
  // the clock would stop an unwatched radar from ever standing down.
  _connect (info) {
  ...
  // Open the spoke stream from mayara for a radar that has an app client,
  // once discovery has told us what the radar looks like.
  _openSource (radarId) {
    const { app, mayaraHost, mayaraPort } = this.ctx
    const r = this._radars.get(radarId)
    if (!r?.info || !this._spokeMsgType || this._stopped) return
    if (this._sources.has(radarId) || !this._clients.get(radarId)?.size) return

    const { info, thetaCount, rhoCount } = r
    ...
    ws.on('close', () => {
      if (this._sources.get(radarId) !== ws) return
      this._sources.delete(radarId)
      if (!this._stopped && this._clients.get(radarId)?.size) {
        app.debug(`Radar ${radarId}: stream closed — retry in ${RETRY_MS}ms`)
        setTimeout(() => this._openSource(radarId), RETRY_MS)
      }
    })
  }

  // The last app client left: let go of the spoke stream so mayara sees
  // nobody watching.
  _closeSource (radarId) {
    const ws = this._sources.get(radarId)
    if (!ws) return
    this._sources.delete(radarId)
    this.ctx.app.debug(`Radar ${radarId}: no app clients — closing spoke stream`)
    ws.close()
  }
