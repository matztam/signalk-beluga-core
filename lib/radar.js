...
class Radar {
  ...
  async _flush (radarId) {
    ...
    try {
      ...
      const frame  = await compress(Buffer.from(packed.buffer, packed.byteOffset, packed.byteLength), 1)
  ...
  _onSpoke (radarId, spoke) {
    ...
    if (clients && clients.size > 0) {
      if (r.pendingSpokes.length >= r.thetaCount) r.pendingSpokes.splice(0)
  ...
  async _discover () {
    ...
    app.debug(`Radar: getRadars() → ${radars.length} radar(s): ${JSON.stringify(radars.map(r => ({ id: r.id, name: r.name, status: r.status, streamUrl: r.streamUrl, spokesPerRevolution: r.spokesPerRevolution, maxSpokeLen: r.maxSpokeLen })))}`)
