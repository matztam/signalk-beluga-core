...
class Ble {
  ...
  _setupAndSpawn () {
    ...
    // If venv exists (with a usable pip — an interrupted first run, e.g. a
    // second start racing the initial venv+pip setup, can leave python3
    // present but pip missing) and all packages importable, go straight to
    // spawn. Otherwise fall through and (re)run the full setup.
    if (fs.existsSync(PYTHON) && fs.existsSync(PIP)) {
